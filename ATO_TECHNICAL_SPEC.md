...
- Model context size is explicit qualified configuration via `TEXT_MODEL_CONTEXT_TOKENS`, `TEXT_MODEL_MAX_OUTPUT_TOKENS`, and `CONTEXT_UTILIZATION_TARGET`.
- `CONTEXT_UTILIZATION_TARGET` is a maximum input-context utilization cap (default `0.70`; `1.0` preserves reserve-only budgeting).
- Always reserve configured output tokens plus 2,048 tokens for instructions and schema overhead.
- Effective input budget is `max(0, min(floor(context × target), context − output − instruction reserve))`.
- Include all linked evidence chunks when they fit.
- When they do not fit, rank chunks deterministically using package-scoped PostgreSQL full-text search for chat and sealed source order for sufficiency_matrix, then fill the remaining budget greedily in rank order.
...
Job, attempt, pending-approval expiry, disposition decision, and the full `/api/v1` contract are published in `docs/contracts/LIFECYCLE_AND_ERRORS.md` and `docs/contracts/domain.schema.json`. The delivered product slice includes OIDC-backed server sessions, the React/Vite portal, systems and package revisions, mixed-format upload with extraction (dev_local substitutes and production fail-closed scanner resolution), package draft editor and sealed confirm, deterministic and model-assisted analysis runs with durable jobs, exact matrix persistence, artifact manifests, review dispositions, hash-bound export approval and download, revision-scoped PostgreSQL full-text search, bounded grounded package chat with `CHAT_*` limits and deterministic refusal, `ato-operator` preflight/migration/qualification/drill/audit tooling, and deployment assets for API, portal nginx, intake worker, and analyzer worker. Alembic head is `20260717_0013`. Phase 5 adds profile-parametrized PostgreSQL workflow integration tests, a sealed qualification corpus manifest, validation-drill and AI-evaluation record contracts, and executable drill dispatchers with append-only records. This remains a contract-tested codebase with `dev_local` substitutes at external boundaries: it is not proof of production malware scanning, customer IdP deployment, live RHEL drills, or AI qualification, and does not close **HS-001** through **HS-006**, **HS-008**, or **HS-009** without customer/authority evidence. Release evidence is indexed in `docs/RELEASE_EVIDENCE_INDEX.md`; Phase 6 documentation reconciliation is recorded in `docs/P6_GATE_RECORD.md`.
