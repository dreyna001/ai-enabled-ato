...
- When the pinned authority manifest is present and digest-valid but not `approved`, `authority_manifest` reports `degraded` and aggregate readiness returns HTTP `503` with `error_code: reconciliation_required`. The current repository manifest is `draft` while **HS-001** is open; local readiness therefore stays degraded/503 until qualified authority review closes **HS-001** and profiles are marked `qualified`.
- Bundled FedRAMP profiles under `reference/profiles/` and customer FISMA profiles compiled through `scripts/compile_fisma_analysis_profile.py` are digest-verified draft artifacts. `onprem_production` rejects draft profiles even when manifest bytes verify.
...
- **Authority package refresh (air-gapped).** Obtain official vendor bytes out of band. Update vendored files under `reference/authorities/` and manifest digests/metadata in `docs/contracts/authority-manifest.json`. Do not hand-edit generated profile rule content or substitute model knowledge for authority text. Review changes with qualified SMEs, approve the authority manifest under **HS-001**, regenerate bundled profiles (`python scripts/compile_analysis_profiles.py --qualification-status qualified` and matching `--check` after approval) and customer FISMA compiles (`python scripts/compile_fisma_analysis_profile.py ... --qualification-status qualified` with approved inventory and matching `--check`), run targeted compiler pytest, execute applicable live-model evaluation, then package the release with updated digests and release notes. The current repository manifest remains `draft`; qualified regeneration commands fail until manifest approval is recorded.
