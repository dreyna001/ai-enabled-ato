...
- **Production API credentials:** [`deployment/systemd/ato-api.service`](../deployment/systemd/ato-api.service) loads `database-dsn`, `audit-hmac-key`, and `oidc-client-secret` through `LoadCredential`. Runtime JSON holds only credential references; never inline secret values.
...
- **Package API authentication:** when `IDENTITY_PROVIDER_MODE=oidc` is configured with `PORTAL_PUBLIC_ORIGIN`, the API loads Postgres-backed sessions from the portal cookie and injects `authenticated_principal` on `/api/v1` routes. Untrusted identity headers are stripped and rejected at the application boundary; mutating auth routes and package routes require CSRF + Origin. Expired sessions and login states are purged through `ato-operator purge-auth`. Without identity configuration, routes remain fail-closed HTTP `401` (**HS-003** / **EP-06** partial).
