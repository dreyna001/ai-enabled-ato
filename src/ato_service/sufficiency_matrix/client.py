"""Model-call wrapper for sufficiency_matrix using model_gateway and text_llm."""
...
from ato_service.sufficiency_matrix.prompt import (
    build_repair_prompt,
    build_system_prompt,
    build_user_prompt,
)
from ato_service.sufficiency_matrix.tokens import estimate_tokens_from_text
from ato_service.sufficiency_matrix.types import FactBundle, ModelCallMetadata
...
class SufficiencyModelRoutingError(Exception):
...
class SufficiencyModelCallError(Exception):
    def __init__(
        self,
        *,
        error_code: str,
...
def sufficiency_model_request(base: ModelCallRequest) -> ModelCallRequest:
    return replace(
        base,
        capability=ModelCapability.SUFFICIENCY_MATRIX,
        max_llm_calls=min(base.max_llm_calls, MAX_LLM_CALLS_PER_BATCH),
    )


async def invoke_sufficiency_model(
    *,
    ...
    if request.capability is not ModelCapability.SUFFICIENCY_MATRIX:
        raise ValueError("capability must be SUFFICIENCY_MATRIX")
    ...
    prompt_text = system + user_content
    ...
    async def _timed_callback() -> str:
        ...
        try:
            ...
            raise SufficiencyModelCallError(
    ...
    try:
    ...
    except SufficiencyModelCallError:
        raise
    except (
        ModelRoutingDeniedError,
        ClassifiedDataUnsupportedError,
        ...
        raise SufficiencyModelRoutingError(
    ...
    metadata = ModelCallMetadata(
        ...
        model_reported=model_requested,
        provider_request_id=str(uuid.uuid4()),
        input_tokens=estimate_tokens_from_text(prompt_text),
        output_tokens=estimate_tokens_from_text(raw),
    )
