...
def load_sp800_60_catalog() -> Sp80060Catalog:
    path = _project_root() / _CATALOG_RELATIVE
    document = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(document, dict):
        raise ValueError("SP 800-60 catalog must be a JSON object")
    items = document.get("information_types")
    if not isinstance(items, list) or not items:
        raise ValueError("SP 800-60 catalog must include information_types")
    parsed: list[Sp80060InformationType] = []
    for entry in items:
        if not isinstance(entry, dict):
            raise ValueError("SP 800-60 catalog entries must be objects")
        ...
        if not identifier or not title:
            raise ValueError("SP 800-60 catalog entries require identifier and title")
        parsed.append(
